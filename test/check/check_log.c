...
void log_test_start(SRunner * sr, TCase * tc, TF * tfun)
{
    ...
    printf(YELLOW"%s"DEFAULT":""%s"DEFAULT, tc->name, tfun->ttest->name);
...
void log_test_end(SRunner * sr, TestResult * tr)
{
	char durStr[25] = "?";
	if(tr->duration > 0)
		formatTime((unsigned long)tr->duration, durStr);

	const int success = tr->rtype == CK_PASS;
	printf(" %s" DEFAULT, success ? GREEN"PASS" : RED"FAIL");
	printf(" (%s)\n", durStr);
	if(!success)
