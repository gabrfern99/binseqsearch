...
void formatTime(unsigned long time, char *output)
{
    char *durStr = "us";
    if(time > 1000)
    {
    ...
    sprintf(output, "%ld%s", time, durStr);
}
...
char *sr_stat_str(SRunner * sr)
{
    ...
    char *result = percent_passed(ts) == 100
        ? GREEN "PASS" DEFAULT
        : RED "FAIL" DEFAULT;

    unsigned long duration = 0; //microseconds
    TestResult **results = srunner_results(sr);
    for(int i=0; i<srunner_ntests_run(sr); i++)
        if(results[i]->duration > 0)
            duration += results[i]->duration;
    free(results);

    char durStr[25];
    formatTime(duration, durStr);
    str = ck_strdup_printf("%s: %d%%: Checks: %d, Failures: %d, Errors: %d\n"
                           "Took %s\n", result,
